...
Function TryRun(ByVal command)
    ...
    Dim rc
    rc = WshShell.Run("cmd /c " & command, 0, True)
...
Sub TryPythonPath(ByVal py)
    ...
    If pyw <> "" And FSO.FileExists(pyw) Then
        If TryRun(Q(pyw) & " launch.py --detach") Then WScript.Quit 0
    End If
    If FSO.FileExists(py) Then
        If TryRun(Q(py) & " launch.py --detach") Then WScript.Quit 0
    End If
End Sub
...
If TryRun("pyw -3 launch.py --detach") Then WScript.Quit 0
If TryRun("py -3 launch.py --detach") Then WScript.Quit 0
If TryRun("pythonw launch.py --detach") Then WScript.Quit 0
If TryRun("python launch.py --detach") Then WScript.Quit 0
